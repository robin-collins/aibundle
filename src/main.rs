...
use std::{collections::{HashSet, HashMap}, fs, io, path::Path, path::PathBuf};
...
impl App {
    ...
    fn format_selected_items(&mut self) -> io::Result<String> {
        ...
        if self.output_format == OutputFormat::Llm {
            ...
            let root_name = self.current_dir.file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("root")
                .to_string();
                    ...
                    if let Some(children) = unsafe { &mut (*parent_ptr).children } {
                        if !children.contains_key(&name) {
                            let mut node = Node {
                                name: name.clone(),
                                is_dir: path.is_dir(),
                                children: if path.is_dir() { Some(HashMap::new()) } else { None },
            ...
            // Generate LLM output
            format_llm_output(&mut output, &file_contents, &self.current_dir, &root_node, &dependencies);
...
fn analyze_dependencies(file_contents: &[(String, String)], _base_dir: &Path) -> std::collections::HashMap<String, FileDependencies> {
    let mut dependencies = std::collections::HashMap::new();
    let mut imports: std::collections::HashMap<String, HashSet<String>> = std::collections::HashMap::new();
        ...
        for pattern in patterns {
            ...
            for cap in regex.captures_iter(content) {
                if let Some(m) = cap.get(1) {
                    imports.get_mut(file_path).unwrap().insert(m.as_str().to_string());
                }
            }
        }
    }
    ...
    for (file_path, _) in file_contents {
        ...
        let name_without_ext = Path::new(&basename).file_stem()
            .and_then(|n| n.to_str())
            .unwrap_or("")
            .to_string();
        ...
        while rel_path.contains('/') {
            ...
            let without_ext = Path::new(&rel_path).file_stem()
                .and_then(|n| n.to_str())
                .unwrap_or("")
                .to_string();
    ...
    for (file_path, imported) in imports {
        ...
        for imp in imported {
            ...
            let import_variations = vec![
                imp.clone(),
                Path::new(&imp).file_name().and_then(|n| n.to_str()).unwrap_or("").to_string(),
                Path::new(&imp).file_stem().and_then(|n| n.to_str()).unwrap_or("").to_string(),
                imp.replace('.', "/"),
        ...
        dependencies.insert(file_path, FileDependencies {
            internal_deps,
            external_deps,
        });
    }
...
fn write_file_tree_to_string(node: &Node, prefix: &str, is_last: bool) -> String {
    ...
    if node.parent.is_some() {  // Skip root node
    ...
    if node.is_dir && node.children.is_some() {
        ...
        let items: Vec<_> = children.iter()
            .sorted_by(|a, b| Ord::cmp(&(!a.1.is_dir, a.0.to_lowercase()), &(!b.1.is_dir, b.0.to_lowercase())))
            .collect();
...
fn format_llm_output(
    ...
    output.push_str(&format!("- **Files included in this analysis**: {}\n", selected_files));
    ...
    if !languages.is_empty() {
        ...
        for (i, (ext, count)) in lang_counts.iter().enumerate() {
            if i >= 5 { break; }  // Show top 5 languages
    ...
    let main_dirs: Vec<_> = root_node.children.as_ref()
        .map(|children| children.values()
    ...
    if !main_dirs.is_empty() {
        ...
        for dir_node in main_dirs {
            let dir_files: Vec<_> = file_contents.iter()
            ...
            if !dir_files.is_empty() {
                ...
                let mut dir_exts: std::collections::HashMap<String, usize> = std::collections::HashMap::new();
    ...
    let mut referenced_by: std::collections::HashMap<String, Vec<String>> = std::collections::HashMap::new();
    for (file, deps) in dependencies {
        for dep in &deps.internal_deps {
            referenced_by.entry(dep.clone())
    ...
    if !referenced_by.is_empty() {
        ...
        for (file, refs) in refs.iter().take(10) {
            if refs.len() > 1 {  // Only files referenced multiple times
                output.push_str(&format!("- **`{}`** is imported by {} files\n", file, refs.len()));
            }
        }
        output.push_str("\n");
    }
    ...
    for (file, deps) in dependencies {
        if !deps.internal_deps.is_empty() || !deps.external_deps.is_empty() {
            ...
            if !deps.internal_deps.is_empty() {
                ...
                sorted_deps.sort();
                let display_deps: Vec<_> = sorted_deps.iter()
                    .take(5)
                    .map(|d| format!("`{}`", d))
                    .collect();
            ...
            if !file_deps.internal_deps.is_empty() || !file_deps.external_deps.is_empty() {
                ...
                if !file_deps.internal_deps.is_empty() {
                    ...
                    if file_deps.internal_deps.len() > 3 {
                        output.push_str(&format!(" and {} more", file_deps.internal_deps.len() - 3));
                    }
                    output.push_str("\n");
                }
                ...
                if !file_deps.external_deps.is_empty() {
                    ...
                    if file_deps.external_deps.len() > 3 {
                        output.push_str(&format!(" and {} more", file_deps.external_deps.len() - 3));
                    }
                    output.push_str("\n");
                }
